...
function cfgString(configuration: Record<string, unknown> | undefined, key: string): string {
  const value = configuration?.[key];
  return typeof value === "string" && value.trim() ? value : "—";
}

function originSummary(configuration: Record<string, unknown> | undefined): {
  address: string;
  port: string;
  hostHeader: string;
} {
  ...
  if (!origin || typeof origin !== "object") {
    return { address: "—", port: "—", hostHeader: "—" };
  }
  const row = origin as Record<string, unknown>;
  return {
    address: typeof row.address === "string" && row.address ? row.address : "—",
    port: row.port != null ? String(row.port) : "—",
    hostHeader: typeof row.host_header === "string" && row.host_header ? row.host_header : "—",
  };
}

export function VarnishDetailPage() {
  ...
  const configuration = instance?.configuration as Record<string, unknown> | undefined;
  const origin = originSummary(configuration);

  const tabs: Array<{ id: Tab; label: string }> = [
    { id: "overview", label: t("varnish.tabs.overview") },
    { id: "networks", label: t("varnish.tabs.networks") },
    { id: "logs", label: t("varnish.tabs.logs") },
  ];
    ...
    <div className="space-y-6">
      <section className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {instance?.last_error ? (
            <p className="mt-2 max-w-2xl text-sm text-danger">{instance.last_error}</p>
          ) : null}
        </div>
        <MutationGate hide>
      ...
      {tab === "overview" ? (
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-3 border border-line bg-paper-elevated p-4">
            <h3 className="font-medium text-ink">{t("varnish.configTitle")}</h3>
            <p className="text-sm text-ink-muted">{t("varnish.configHint")}</p>
            <dl className="space-y-2 font-mono text-sm">
              <div>
                <dt className="text-ink-muted">{t("varnish.bind")}</dt>
                <dd>{cfgString(configuration, "bind")}</dd>
              </div>
              <div>
                <dt className="text-ink-muted">{t("varnish.storage")}</dt>
                <dd>{cfgString(configuration, "storage_size")}</dd>
              </div>
              <div>
                <dt className="text-ink-muted">{t("varnish.ttl")}</dt>
                <dd>{cfgString(configuration, "ttl_default")}</dd>
              </div>
            </dl>
          </div>
          <div className="space-y-3 border border-line bg-paper-elevated p-4">
            <h3 className="font-medium text-ink">{t("varnish.originTitle")}</h3>
            <dl className="space-y-2 font-mono text-sm">
              <div>
                <dt className="text-ink-muted">{t("varnish.originAddress")}</dt>
                <dd>{origin.address}</dd>
              </div>
              <div>
                <dt className="text-ink-muted">{t("varnish.originPort")}</dt>
                <dd>{origin.port}</dd>
              </div>
              <div>
                <dt className="text-ink-muted">{t("varnish.hostHeader")}</dt>
                <dd>{origin.hostHeader}</dd>
              </div>
            </dl>
          </div>
        </div>
      ...
      {tab === "networks" ? (
        <div className="space-y-4 border border-line bg-paper-elevated p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-muted">{t("varnish.networksHint")}</p>
          </div>
          <MutationGate>
            <form
