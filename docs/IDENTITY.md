...
- **Auto layout** bruker **ELK.js** (traffic LTR, process TTB, tree, star, multi-site swimlanes, compact, selected). Preferanser: preserve groups/pinned, animate, fit view, snap-to-grid. Høyreklikk **Pin position**. **Placement Domains** er design-lokale (registry `graph_json.placementDomains` + lane-noder); grupper peker på `placementDomainId`. Manuell «Add lane» legger tomme Site/Shared-lanes uten å re-layout gruppeinnhold. HAProxy-instanser kan ha valgfri `configuration.site` (Settings) som Designer mapper til domain ved drop — ingen global Sites-API eller automatisk deploy-routing ennå.
