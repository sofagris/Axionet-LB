...
- Mens Designer er åpen, **polles** live-config for linkede HAProxy-grupper (~8 s). Endret fingerprint re-hydrerer gruppen og **overskriver** canvas-innholdet for den gruppen (manuell «Oppdater fra instans» finnes også). Lokal canvas-redigering på linket gruppe kan derfor gå tapt til sync. Senere: revisjonskvalitetssjekk og diff før overwrite.
