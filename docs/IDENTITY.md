...
- Når du drar inn en **eksisterende HAProxy-instans**, hydreres gruppen med live frontends / backends / servers / error pages fra API (ikke bare et default-skjelett). `toHaproxyDesiredState` i frontend mapper forbereder senere Designer→deploy; Apply deployer fortsatt ikke.
- Mens Designer er åpen, **polles** live-config for linkede HAProxy-grupper (~8 s). Endret fingerprint re-hydrerer gruppen; **eksisterende node-posisjoner beholdes** (match på rolle+navn). Manuell «Oppdater fra instans» finnes også. Save remounter ikke canvas / tømmer ikke sync-fingerprints.
- **Auto layout** bruker **ELK.js** (traffic LTR, process TTB, tree, star, multi-site swimlanes, compact, selected). Preferanser: preserve groups/pinned, animate, fit view, snap-to-grid. Høyreklikk **Pin position**.
- **Sites / Placement Domains / LB-inventar** lever under **Settings → Sites** (plattform-API). **Site** = fysisk sted for en Axionet-LB-boks (kan finnes uten LB). **Placement Domain** (`site` | `shared`) er Designer-/deploy-taksonomi. **LB** = Axionet-LB-apparat (inventar: navn, beskrivelse, IP, site); denne installasjonen er merket lokal. HAProxy-instanser arver site implicit via boksen — ikke eget `configuration.site`. Designer leser Placement Domains fra API; drop av HAProxy foreslår domain ut fra lokal LBs site. Ingen multi-LB federation eller automatisk deploy-routing ennå.
