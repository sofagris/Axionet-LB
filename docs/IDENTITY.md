...
- **Auto layout** bruker **ELK.js** (traffic LTR, process TTB, tree, star, multi-site swimlanes, compact, selected). Preferanser: preserve groups/pinned, animate, fit view, snap-to-grid. Høyreklikk **Pin position**. Grupper har valgfritt **Placement domain** (Site A / Shared Services / …) som grunnlag for swimlanes — feltet ligger i `graph_json` mot en senere deploybar multi-site-modell (ingen Placement Domain API ennå).
