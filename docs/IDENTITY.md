...
- Mens Designer er åpen, **polles** live-config for linkede HAProxy-grupper (~8 s). Endret fingerprint re-hydrerer gruppen; **eksisterende node-posisjoner beholdes** (match på rolle+navn). Manuell «Oppdater fra instans» finnes også. Save remounter ikke canvas / tømmer ikke sync-fingerprints. Toolbar: layout-modus (flyt/rutenett/stabel) + Auto layout (scope = valgt gruppe eller top-level).
